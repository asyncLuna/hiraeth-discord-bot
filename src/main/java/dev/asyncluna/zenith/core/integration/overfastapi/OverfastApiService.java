...
public class OverfastApiService {
  ...
  public Flux<HeroShort> getHeroes(
    ...
    return client
        .get(OverfastApiEndpoint.GET_A_LIST_OF_HEROES, cacheKey, uriFunction, HeroShort[].class)
  ...
  public Flux<HeroStatsSummary> getHeroStats(
    ...
    return client
        .get(OverfastApiEndpoint.GET_HERO_STATS, cacheKey, uriFunction, HeroStatsSummary[].class)
        .contextWrite(context -> context.put(HttpUtils.PARAM_CONTEXT_KEY, queryParams.asMap()))
        .flatMapMany(Flux::fromArray);
  }
  ...
  public Flux<Map> getMaps(String gamemode) {
    ...
    return client
        .get(OverfastApiEndpoint.GET_A_LIST_OF_MAPS, cacheKey, uriFunction, Map[].class)
        .contextWrite(context -> context.put(HttpUtils.PARAM_CONTEXT_KEY, queryParams.asMap()))
        .flatMapMany(Flux::fromArray);
  }
  ...
  public Mono<Hero> getHeroData(String heroKey, OverfastApiQueryParam.Locale locale) {
    ...
    return client
        .get(OverfastApiEndpoint.GET_HERO_DATA, cacheKey, uriFunction, Hero.class)
        .contextWrite(context -> context.put(HttpUtils.PARAM_CONTEXT_KEY, queryParams.asMap()));
  }
  ...
  public Flux<RoleDetail> getRoles(OverfastApiQueryParam.Locale locale) {
    ...
    OverfastApiQueryParams queryParams = OverfastApiQueryParams.builder().locale(locale).build();
    ...
    return client
        .get(OverfastApiEndpoint.GET_A_LIST_OF_ROLES, cacheKey, uriFunction, RoleDetail[].class)
        .contextWrite(context -> context.put(HttpUtils.PARAM_CONTEXT_KEY, queryParams.asMap()))
        .flatMapMany(Flux::fromArray);
  }
  ...
  public Mono<PlayerCareerStats> getPlayerCareerStats(String playerId, String gamemode) {
    log.info(
        "Fetching career stats from Overfast API for playerId={}, gamemode={}", playerId, gamemode);
    String formattedPlayerId = normalizePlayerId(playerId);
    String cacheKey =
        String.format("playerCareerStats:id:%s:gamemode:%s", formattedPlayerId, gamemode);
    ...
    return client
        .get(OverfastApiEndpoint.GET_PLAYER_CAREER_STATS, cacheKey, uriFunction, JsonNode.class)
