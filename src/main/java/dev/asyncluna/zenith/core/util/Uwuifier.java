...
public class Uwuifier {
    ...
    private static final List<String> NSFW_ACTIONS_LIST = List.of(
            "***nuzzles your necky wecky***",
            "***licks lips***",
            "***glomps and huggles***",
            "***glomps***",
    ...
    private static final List<String> QUESTION_EXCLAMATIONS = List.of("?", "??", "???", "!?", "?!!", "?!?1", "?!?!");
    ...
    private static final List<UwuPattern> UWU_PATTERNS = List.of(
            new UwuPattern("[rl]", "w"),
            new UwuPattern("[RL]", "W"),
    ...
    private String uwuifyWords(String msg) {
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            for (UwuPattern pattern : UWU_PATTERNS) word = word.replaceAll(pattern.regex, pattern.replacement);
            words[i] = word;
        }
    ...
    private String uwuifySpaces(String msg) {
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            if (random.nextDouble() <= STUTTER_CHANCE) {
                ...
                for (int j = 0; j < stutterCount; j++)
                    stringBuilder.append(firstChar).append("-");
            }
            ...
            if (random.nextDouble() <= ACTION_CHANCE) {
                ...
                stringBuilder.append(" ").append(availableActions.get(random.nextInt(availableActions.size())));
            }
