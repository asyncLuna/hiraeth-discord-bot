...
public class MemberOfTheWeekCommand implements BotCommand {
    ...
    private Mono<?> votes(CommandContext ctx) {
        ...
        Mono<User> member =
                ctx.getOptionAsUser("member").orElseThrow(() -> new CommandException("A member must be selected."));

        return member.flatMap(selectedMember -> {
            String memberId = selectedMember.getId().asString();

            if ("all".equals(requestedRound)) {
                return roundService.getAllVoteCount(guildId, memberId).flatMap(count -> ctx.editReply()
                        .withEmbeds(formatVoteCount(ctx, "member_of_the_week.votes.all_title", null, memberId, count)));
            }

            Mono<MemberOfTheWeekRound> roundMono = requestedRound != null
                    ? roundService.getRound(guildId, requestedRound)
                    : roundService.getCurrentRoundForGuild(guildId).switchIfEmpty(roundService.getLatestRound(guildId));

            return roundMono
                    .switchIfEmpty(Mono.error(new CommandException(ctx.localize("member_of_the_week.error.no_rounds"))))
                    .flatMap(round -> roundService
                            .getVoteCount(guildId, round.getId(), memberId)
                            .flatMap(count -> ctx.editReply()
                                    .withEmbeds(formatVoteCount(
                                            ctx,
    ...
    private EmbedCreateSpec formatVoteCount(
        ...
        String title = titleArgument == null ? ctx.localize(titleKey) : ctx.localize(titleKey, titleArgument);
        return EmbedCreateSpec.builder()
                .color(EmbedUtils.DEFAULT_COLOR)
                .title(title)
                .description(ctx.localize("member_of_the_week.votes.description", "<@" + memberId + ">", count))
    ...
    public Mono<Void> autocomplete(ChatInputAutoCompleteEvent event) {
        ...
        List<ApplicationCommandOptionChoiceData> allChoice = input.isBlank() || "all".contains(input)
                ? List.of(ApplicationCommandOptionChoiceData.builder()
        ...
        return roundService
                .getRounds(guildId)
                .filter(round ->
                        round.getId() != null && round.getId().toLowerCase().contains(input))
                .take(DiscordConstants.MAX_AUTO_COMPLETE_RESULTS - allChoice.size())
                .map(round -> ApplicationCommandOptionChoiceData.builder()
    ...
    private Mono<?> pause(CommandContext ctx) {
        return roundService.pauseVoting().then(ctx.editReply(ctx.localize("member_of_the_week.paused")));
    }

    private Mono<?> unpause(CommandContext ctx) {
        return roundService.unpauseVoting().then(ctx.editReply(ctx.localize("member_of_the_week.unpaused")));
    }
    ...
    private Mono<?> open(CommandContext ctx) {
        return roundService
                ...
                .flatMap(paused -> paused
                        ? Mono.error(new CommandException(ctx.localize("member_of_the_week.error.paused")))
                        : roundService.getCurrentRound())
                .flatMap(round -> Mono.error(
                        new CommandException(ctx.localize("member_of_the_week.error.already_open", round.getId()))))
                .switchIfEmpty(roundService
                        .openRound()
                        .flatMap(round -> ctx.editReply(ctx.localize("member_of_the_week.opened", round.getId()))));
    }
    ...
    private Mono<?> close(CommandContext ctx) {
        return roundService
                ...
                .switchIfEmpty(
                        Mono.error(new CommandException(ctx.localize("member_of_the_week.error.no_open_round"))));
    }
