package dev.asyncluna.zenith.discord.feature.animal;

import dev.asyncluna.zenith.core.integration.animalapi.AnimalApiService;
import dev.asyncluna.zenith.core.integration.animalapi.AnimalApiType;
...
public class AnimalCommand implements BotCommand {
    ...
    public Mono<?> handle(CommandContext ctx) {
        ...
        if (animalOption.isEmpty()) return Mono.error(new CommandException(ctx.localize("animal.error.missing_type")));
    ...
    public Mono<Void> autocomplete(ChatInputAutoCompleteEvent event) {
        ...
        Mono<List<ApplicationCommandOptionChoiceData>> choicesMono = optionName.equals("type")
                ? Flux.fromArray(AnimalApiType.values())
                        .filter(type -> type.getApiName().toLowerCase().contains(userInput)
                                || type.getFriendlyName().toLowerCase().contains(userInput))
                        .take(DiscordConstants.MAX_AUTO_COMPLETE_RESULTS)
                        .map(type -> (ApplicationCommandOptionChoiceData) ApplicationCommandOptionChoiceData.builder()
