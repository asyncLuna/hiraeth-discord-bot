...
public class MapCommand implements BotCommand {
    ...
    @Getter
    private final Cache<String, MapSession> sessionCache = Caffeine.newBuilder()
    ...
    public Mono<?> handle(CommandContext ctx) {
        String gamemode = ctx.getOptionAsString("gamemode")
                .orElseThrow()
                .toLowerCase()
                .replace("_", "-")
                .replace(" ", "-");
        ...
        return overfastApiService.getMaps(gamemode).collectList().flatMap(maps -> {
            if (maps.isEmpty()) {
    ...
    public Mono<Void> autocomplete(ChatInputAutoCompleteEvent event) {
        ...
        Mono<List<ApplicationCommandOptionChoiceData>> choicesMono = optionName.equals("gamemode")
                ? overfastApiService
                        .getMaps()
    ...
    private EmbedCreateSpec createMapEmbed(Map map, CommandContext ctx) {
        ...
        if (map.gamemodes() != null && !map.gamemodes().isEmpty()) {
            String modesList = map.gamemodes().stream()
