...
import dev.asyncluna.zenith.core.integration.overfastapi.dto.HitPoints;
...
public class HeroStatsButtonListener implements EventListener<ButtonInteractionEvent> {
    ...
    public Mono<Void> execute(ButtonInteractionEvent event) {
        ...
        return guildSettingsProvider
                ...
                .flatMap(settings -> {
                    ...
                    return overfastApiService.getHeroData(heroKey, null).flatMap(heroData -> {
                        EmbedCreateSpec detailedEmbed = createHeroDataEmbed(heroData, currentLocale);
                        ...
                        return event.edit().withEmbeds(detailedEmbed).withComponents(ActionRow.of(backButton));
                    });
                })
                .then();
    }
    ...
    private Mono<Void> handleGoBack(
            ButtonInteractionEvent event, HeroStatsSession session, String sessionId, Locale currentLocale) {
        ...
        return Mono.zip(statsMono, heroesMapMono, mapsNameMapMono).flatMap(tuple -> {
            ...
            HeroStatsSummary currentHeroStats = heroStatsList.stream()
            ...
            String friendlyMapName = session.map() != null
                    ? mapsNameMap.getOrDefault(session.map().toLowerCase(), session.map())
                    : null;

            EmbedCreateSpec.Builder backEmbedBuilder = EmbedCreateSpec.builder()
                    .title(
            ...
            if (currentHeroDetails != null
                    && currentHeroDetails.role() != null
                    && !currentHeroDetails.role().isBlank())
                backEmbedBuilder.addField(
                        i18nManager.localize("hero.role", currentLocale),
                        WordUtils.capitalizeFully(currentHeroDetails.role()),
                        true);
            ...
            if (friendlyMapName != null)
                backEmbedBuilder.addField(i18nManager.localize("hero.map", currentLocale), friendlyMapName, true);
            ...
            backEmbedBuilder
                    .addField(
                            ...
                            (currentHeroStats.pickrate() != null ? currentHeroStats.pickrate() + "%" : "N/A"),
                            true)
                    .addField(
                            i18nManager.localize("hero.winrate", currentLocale),
                            (currentHeroStats.winrate() != null ? currentHeroStats.winrate() + "%" : "N/A"),
                            true)
                    .addField(
                            i18nManager.localize("hero.banrate", currentLocale),
                            (currentHeroStats.banrate() != null ? currentHeroStats.banrate() + "%" : "N/A"),
                            true);
            ...
            String selectAHeroTemplate = i18nManager.localize("hero.select_a_hero", currentLocale);

            if (heroStatsList.size() <= DiscordConstants.MAX_COMPONENT_ENTRIES) {
                SelectMenu selectMenu = SelectMenu.of(
                                "hs-1:" + sessionId,
                                heroStatsList.stream()
                                        .map(heroStatsSummary -> {
                                            HeroShort details = heroesMap.get(
                                                    heroStatsSummary.hero().toLowerCase());
                                            String label = details != null ? details.name() : heroStatsSummary.hero();
                        ...
                        .withPlaceholder(String.format(
                                ...
                                heroesMap.containsKey(
                                                heroStatsList.getFirst().hero().toLowerCase())
                                        ? heroesMap
                                                .get(heroStatsList
                                                        .getFirst()
                                                        .hero()
                                ...
                                heroesMap.containsKey(
                                                heroStatsList.getLast().hero().toLowerCase())
                                        ? heroesMap
                                                .get(heroStatsList
                                                        .getLast()
                                                        .hero()
            ...
            List<HeroStatsSummary> firstHalfStats = heroStatsList.stream()
                    .limit(DiscordConstants.MAX_COMPONENT_ENTRIES)
                    .toList();
            List<HeroStatsSummary> secondHalfStats = heroStatsList.stream()
                    .skip(DiscordConstants.MAX_COMPONENT_ENTRIES)
                    .limit(DiscordConstants.MAX_COMPONENT_ENTRIES)
                    .toList();

            SelectMenu firstSelectMenu = SelectMenu.of(
                            "hs-1:" + sessionId,
                    ...
                    .withPlaceholder(String.format(
                            selectAHeroTemplate,
                            heroesMap.containsKey(
                                            firstHalfStats.getFirst().hero().toLowerCase())
                                    ? heroesMap
                                            .get(firstHalfStats
                                                    .getFirst()
                                                    .hero()
                            ...
                            heroesMap.containsKey(
                                            firstHalfStats.getLast().hero().toLowerCase())
                                    ? heroesMap
                                            .get(firstHalfStats.getLast().hero().toLowerCase())
                                            .name()
                                    : firstHalfStats.getLast().hero()));

            SelectMenu secondSelectMenu = SelectMenu.of(
                            "hs-2:" + sessionId,
                    ...
                    .withPlaceholder(String.format(
                            selectAHeroTemplate,
                            heroesMap.containsKey(
                                            secondHalfStats.getFirst().hero().toLowerCase())
                                    ? heroesMap
                                            .get(secondHalfStats
                                                    .getFirst()
                                                    .hero()
                            ...
                            heroesMap.containsKey(
                                            secondHalfStats.getLast().hero().toLowerCase())
                                    ? heroesMap
                                            .get(secondHalfStats
                                                    .getLast()
                                                    .hero()
            ...
            return event.edit()
                    .withEmbeds(backEmbedBuilder.build())
                    .withComponents(
                            ActionRow.of(detailsButton), ActionRow.of(firstSelectMenu), ActionRow.of(secondSelectMenu));
        });
    }
    ...
    private EmbedCreateSpec createHeroDataEmbed(Hero heroData, Locale currentLocale) {
        ...
        if (!StringUtils.isBlank(heroData.description())) embedBuilder.description(heroData.description());

        if (!StringUtils.isBlank(heroData.role()))
            embedBuilder.addField(
                    i18nManager.localize("hero.role", currentLocale), WordUtils.capitalizeFully(heroData.role()), true);
