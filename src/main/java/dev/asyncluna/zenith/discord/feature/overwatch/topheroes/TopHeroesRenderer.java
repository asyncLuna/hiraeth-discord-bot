...
public class TopHeroesRenderer {
    ...
    public EmbedCreateSpec render(TopHeroesSession session, Locale locale) {
        ...
        EmbedCreateSpec.Builder builder = EmbedCreateSpec.builder()
                ...
                .description(i18nManager.localize(
    ...
    public int totalPages(TopHeroesSession session) {
        return Math.max(
                1,
                (ranked(session.stats(), TopHeroesCommand.findStat(session.statPath()))
                                        .size()
                                + PAGE_SIZE
                                - 1)
                        / PAGE_SIZE);
    ...
    private static List<Map.Entry<String, JsonNode>> ranked(PlayerCareerStats stats, TopHeroesCommand.Stat stat) {
        return stats.heroes().entrySet().stream()
                ...
                .sorted(Comparator.comparingDouble((Map.Entry<String, JsonNode> e) ->
