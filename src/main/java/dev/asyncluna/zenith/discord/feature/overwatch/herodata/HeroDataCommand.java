...
import dev.asyncluna.zenith.discord.feature.overwatch.herostats.HeroStatsCommand;
...
public class HeroDataCommand implements BotCommand {
    ...
    @Getter
    private final Cache<String, HeroDataSession> sessionCache = Caffeine.newBuilder()
            .expireAfterWrite(15, TimeUnit.MINUTES)
            .maximumSize(1000)
            .build();

    public record HeroDataSession(String userId, String heroKey, OverfastApiQueryParam.Locale locale) {}

    @Override
    public Mono<?> handle(CommandContext ctx) {
        String heroKey = ctx.getOptionAsString("hero").orElseThrow().toLowerCase();
        OverfastApiQueryParam.Locale locale = ctx.getOptionAsString("locale")
                .map(OverfastApiQueryParam.Locale::fromApiName)
                .orElse(null);
        ...
        return overfastApiService.getHeroData(heroKey, locale).flatMap(heroData -> {
            EmbedCreateSpec embed = createHeroDataEmbed(heroData, locale, ctx.getGuildSettings(), ctx.getI18nManager());

            String viewAbilitiesLabel = ctx.localize("hero.view_abilities");
            Button abilitiesButton = Button.primary("hd-abilities:" + sessionId, viewAbilitiesLabel);

            String viewPerksLabel = ctx.localize("hero.view_perks");
            Button perksButton = Button.secondary("hd-perks:" + sessionId, viewPerksLabel);

            return ctx.editReply().withEmbeds(embed).withComponents(ActionRow.of(abilitiesButton, perksButton));
        });
    }
        ...
        Mono<List<ApplicationCommandOptionChoiceData>> choicesMono =
                switch (optionName) {
                    case "hero" -> overfastApiService
                            ...
                            .take(25)
                            .collectList();
                    case "locale" -> Mono.just(OverfastApiQueryParam.Locale.values())
                            .flatMapMany(Flux::fromArray)
                            .filter(locale ->
                                    locale.getFriendlyName().toLowerCase().contains(userInput))
                            .map(locale ->
                                    (ApplicationCommandOptionChoiceData) ApplicationCommandOptionChoiceData.builder()
    ...
    public EmbedCreateSpec createHeroDataEmbed(
            Hero heroData, OverfastApiQueryParam.Locale locale, GuildSettings guildSettings, I18nManager i18nManager) {
        ...
        if (!StringUtils.isBlank(heroData.role()))
            embedBuilder.addField(
                    ...
                    heroStatsCommand.getRoleAsEmoji(heroData.role()) + " " + WordUtils.capitalizeFully(heroData.role()),
                    true);
        ...
        if (locale != null)
            embedBuilder.footer(
                    i18nManager.localize("locale", currentLocale, locale) + ": " + locale.getFriendlyName(), null);

        return embedBuilder.build();
    }

    public String formatHitPoints(HitPoints hitPoints, GuildSettings guildSettings, I18nManager i18nManager) {
        Locale currentLocale =
                SupportedLocale.forLanguageTag(guildSettings.getLocale()).getLocale();
