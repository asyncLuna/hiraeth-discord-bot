...
public class TopHeroesCommand implements BotCommand {
  private static final List<Stat> STATS =
      List.of(
          ...
          new Stat(
              "best.defensive_assists_most_in_game", "Defensive assists - most in game", false),
          new Stat(
              "best.offensive_assists_most_in_game", "Offensive assists - most in game", false),
          new Stat(
              "best.environmental_kills_most_in_game", "Environmental kills - most in game", false),
          new Stat(
              "best.melee_final_blows_most_in_game", "Melee final blows - most in game", false),
          new Stat(
              "best.time_spent_on_fire_most_in_game", "Time spent on fire - most in game", true),
          ...
          new Stat(
              "average.objective_kills_avg_per_10_min", "Objective kills - avg per 10 min", false),
          new Stat(
              "average.objective_time_avg_per_10_min", "Objective time - avg per 10 min", true),
          new Stat(
              "average.hero_damage_done_avg_per_10_min",
          ...
          new Stat(
              "average.time_spent_on_fire_avg_per_10_min",
              "Time spent on fire - avg per 10 min",
              true),
          new Stat(
              "average.objective_contest_time_avg_per_10_min",
    ...
    return overfastApiService
        ...
        .map(
            stats -> {
              ...
              TopHeroesSession session =
                  new TopHeroesSession(
                      ctx.getAuthor().getId().asString(),
                      playerId,
                      mode,
                      stat.path,
                      stats,
        ...
        .flatMap(
            view ->
                renderer.totalPages(view.session()) > 1
                    ? ctx.editReply()
                        .withEmbeds(renderer.render(view.session(), ctx.getLocale()))
                        .withComponents(
                            componentFactory.createPageButtons(
  ...
  public Mono<Void> autocomplete(ChatInputAutoCompleteEvent event) {
    ...
    List<ApplicationCommandOptionChoiceData> choices =
        switch (name) {
          case "gamemode" ->
              List.of(choice("Quick Play", "quickplay"), choice("Competitive Play", "competitive"));
          case "stat" ->
              STATS.stream()
