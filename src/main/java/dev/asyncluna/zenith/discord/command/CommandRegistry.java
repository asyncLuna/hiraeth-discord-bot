...
public class CommandRegistry {
    ...
    @Getter
    private final List<ApplicationCommandRequest> commandRequests = new ArrayList<>();
    ...
    public CommandRegistry(List<BotCommand> commandBeans) {
        ...
        for (BotCommand command : commandBeans) {
            ...
            List<ApplicationCommandOptionData> options = Arrays.stream(
                            command.getClass().getAnnotationsByType(CommandOption.class))
                    .map(this::mapOption)
                    .toList();
            ...
            if (meta.defaultMemberPermissions() != null
                    && !meta.defaultMemberPermissions().isBlank())
    ...
    private ApplicationCommandOptionData mapOption(CommandOption option) {
        List<ApplicationCommandOptionData> subCommands = Arrays.stream(option.subCommands())
                .map(this::mapSubCommandOption)
                .toList();
