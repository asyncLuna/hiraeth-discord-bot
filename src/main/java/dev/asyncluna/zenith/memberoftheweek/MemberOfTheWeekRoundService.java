...
public class MemberOfTheWeekRoundService {
  ...
  public Mono<MemberOfTheWeekRound> getCurrentRoundForGuild(String guildId) {
    return roundRepository.findFirstByGuildIdAndStatusOrderByStartsAtDesc(
        guildId, MemberOfTheWeekRoundStatus.OPEN);
  }
  ...
  public Mono<Long> getAllVoteCount(String guildId, String candidateId) {
    return mongoTemplate.count(
        Query.query(Criteria.where("guildId").is(guildId).and("candidateId").is(candidateId)),
        "member_of_the_week_votes");
  }
  ...
  private Mono<Message> sendVotingMessage(MemberOfTheWeekRound round) {
    return getGuildLocale()
        .flatMap(
            locale -> {
              SelectMenu selectMenu =
                  SelectMenu.ofUser(COMPONENT_ID_PREFIX + round.getId(), Collections.emptyList())
                      .withPlaceholder(
                          i18nManager.localize("member_of_the_week.select_member", locale))
                      .withMinValues(1)
                      .withMaxValues(1);
  ...
  private Mono<Void> announceResults(List<MemberOfTheWeekVoteCount> winners, long totalVotes) {
    MemberOfTheWeekVoteCount selectedWinner =
        winners.isEmpty()
            ? null
            : (winners.size() == 1 ? winners.getFirst() : pickRandomWinner(winners));
    return getGuildLocale()
        .flatMap(
            locale -> {
              String description =
                  createResultsDescription(winners, selectedWinner, totalVotes, locale);
  ...
  private Mono<Long> countVotes(String roundId) {
    return mongoTemplate.count(
        Query.query(Criteria.where("roundId").is(roundId)), "member_of_the_week_votes");
  }
