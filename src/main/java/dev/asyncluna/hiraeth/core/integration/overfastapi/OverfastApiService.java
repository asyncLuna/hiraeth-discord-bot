...
import dev.asyncluna.hiraeth.core.integration.overfastapi.dto.GamemodeDetails;
...
import dev.asyncluna.hiraeth.core.integration.overfastapi.dto.Map;
import dev.asyncluna.hiraeth.core.integration.overfastapi.dto.PlayerSummary;
import dev.asyncluna.hiraeth.core.integration.overfastapi.dto.RoleDetail;
import dev.asyncluna.hiraeth.core.util.HttpUtils;
...
public class OverfastApiService {
  ...
  public Flux<Map> getMaps(String gamemode) {
    log.debug("Fetching maps from Overfast API with gamemode={}", gamemode);
  ...
  public Flux<RoleDetail> getRoles(OverfastApiQueryParam.Locale locale) {
    log.debug("Fetching roles from Overfast API with locale={}", locale);
  ...
  public Mono<PlayerSummary> getPlayerSummary(String playerId) {
    log.debug("Fetching player summary from Overfast API for playerId={}", playerId);

    String formattedPlayerId = playerId.replace("#", "-");
