...
import dev.asyncluna.hiraeth.core.integration.overfastapi.dto.PlayerSummary;
import dev.asyncluna.hiraeth.core.integration.overfastapi.dto.RoleRank;
...
@Command(
    name = "player_summary",
    description = "Get summary and competitive ranks for an Overwatch player.")
@CommandOption(
    name = "player_id",
...
public class PlayerSummaryCommand implements BotCommand {
  ...
  private EmbedCreateSpec createPlayerSummaryEmbed(
      PlayerSummary playerSummary, CommandContext ctx) {
    ...
    if (playerSummary.competitive() != null) {
      ...
      if (playerSummary.competitive().console() != null) {
        String consoleRanks = formatRankContainer(playerSummary.competitive().console(), ctx);
        embedBuilder.addField(
  ...
  private String formatRankContainer(PlayerCompetitiveRank rank, CommandContext ctx) {
    ...
    appendRoleRank(
        rankContainerBuilder, "tank", ctx.localize("competitive.role.tank"), rank.tank());
    appendRoleRank(
        rankContainerBuilder, "damage", ctx.localize("competitive.role.damage"), rank.damage());
    appendRoleRank(
        rankContainerBuilder, "support", ctx.localize("competitive.role.support"), rank.support());
    appendRoleRank(
        rankContainerBuilder, "open", ctx.localize("competitive.role.open"), rank.open());
  ...
  private void appendRoleRank(
      StringBuilder builder, String roleKey, String roleLabel, RoleRank roleRank) {
    if (roleRank != null && roleRank.division() != null && !roleRank.division().isBlank()) {
