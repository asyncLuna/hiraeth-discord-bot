...
import dev.asyncluna.hiraeth.discord.command.SubCommand;
...
public class UwuLockCommand implements BotCommand {
  ...
  private Mono<?> handleApply(CommandContext ctx) {
    return ctx.getOptionAsUser("user")
        .orElseGet(
            () -> Mono.error(new CommandException(ctx.localize("uwulock.error.invalid_user"))))
        .flatMap(
            user ->
                uwuLockService
                    .lockUser(user.getId().asString())
                    .flatMap(
                        success -> {
                          if (!success) {
                            return Mono.error(
                                new CommandException(
                                    ctx.localize(
                                        "uwulock.apply.already_locked", user.getMention())));
                          }
                          String description =
                              ctx.localize("uwulock.apply.success", user.getMention());
                ...
                uwuLockService
                    ...
                    .flatMap(
                        success -> {
                          ...
                          String description =
                              ctx.localize("uwulock.remove.success", user.getMention());
                          return ctx.editReply()
                              .withEmbeds(
                                  EmbedCreateSpec.builder()
                                      .color(EMBED_COLOR)
  ...
  private Mono<?> handleList(CommandContext ctx) {
    ...
    return uwuLockService
        ...
        .onErrorResume(
            exception -> Mono.error(new CommandException(ctx.localize("uwulock.error.generic"))));
  }
}
