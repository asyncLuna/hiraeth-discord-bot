...
import dev.asyncluna.hiraeth.discord.util.DiscordUtils;
import dev.asyncluna.hiraeth.discord.util.NumberUtils;
        ...
        .flatMap(
            limit ->
                ctx.getEvent()
                    ...
                    .flatMap(
                        channel ->
                            DiscordUtils.requirePermissions(
                                    channel,
                                ...
                                .thenMany(
                                    Flux.defer(
                                        () -> {
                                          Optional<Mono<User>> authorMonoOpt =
                                              ctx.getOptionAsUser("author");
                                          Mono<Optional<Snowflake>> authorIdOptMono =
                                              authorMonoOpt
                                                  .map(
                                                      userMono ->
                                                          userMono
                                          ...
                                          return authorIdOptMono.flatMapMany(
                                              authorOpt ->
                                                  channel
                                                      .getMessagesBefore(
                                                          Snowflake.of(Long.MAX_VALUE))
                                                      .filter(
                                                          filterMessage(authorOpt.orElse(null))));
                                        }))
                                  ...
                                  messageIds -> {
                                    ...
                                    return ctx.getEvent()
                                        ...
                                        .flatMap(
                                            channel ->
                                                channel
                                                    .bulkDelete(Flux.fromIterable(messageIds))
                                                    .count()
                                                    .map(
                                                        messagesNotDeleted ->
                                                            Math.max(
