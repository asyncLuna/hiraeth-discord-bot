...
import dev.asyncluna.hiraeth.discord.command.commands.overwatch.MapCommand;
import dev.asyncluna.hiraeth.discord.command.commands.overwatch.MapCommand.MapSession;
...
public class MapSelectListener implements EventListener<SelectMenuInteractionEvent> {
  ...
  public Mono<Void> execute(SelectMenuInteractionEvent event) {
    ...
    return guildSettingsRepository
        ...
        .onErrorResume(
            exception -> {
              log.error("Database fallback triggered inside MapSelectListener", exception);
