...
import dev.asyncluna.hiraeth.core.integration.overfastapi.dto.Ability;
import dev.asyncluna.hiraeth.core.integration.overfastapi.dto.Hero;
import dev.asyncluna.hiraeth.core.integration.overfastapi.dto.Perk;
...
import dev.asyncluna.hiraeth.discord.command.commands.overwatch.HeroDataCommand;
import dev.asyncluna.hiraeth.discord.command.commands.overwatch.HeroDataCommand.HeroDataSession;
...
public class HeroDataButtonListener implements EventListener<ButtonInteractionEvent> {
  ...
  public Mono<Void> execute(ButtonInteractionEvent event) {
    ...
    if (!"hd-abilities".equals(btnPrefix)
        && !"hd-perks".equals(btnPrefix)
        && !"hd-back".equals(btnPrefix)) return Mono.empty();
    ...
    return guildSettingsRepository
        ...
        .onErrorResume(
            exception -> {
              log.error("Database fallback triggered inside HeroDataButtonListener", exception);
                  ...
                  .flatMap(
                      heroData -> {
                        if ("hd-back".equals(btnPrefix)) {
                          EmbedCreateSpec dataEmbed =
                              heroDataCommand.createHeroDataEmbed(
                          ...
                          String viewPerksLabel =
                              i18nManager.localize("hero.view_perks", currentLocale);
                          Button perksButton =
                              Button.secondary("hd-perks:" + sessionId, viewPerksLabel);
                        ...
                        if ("hd-abilities".equals(btnPrefix)) {
                          EmbedCreateSpec abilitiesEmbed =
                              createHeroAbilitiesEmbed(heroData, currentLocale);
  ...
  private EmbedCreateSpec createHeroAbilitiesEmbed(Hero heroData, Locale currentLocale) {
    ...
    if (heroData.portrait() != null && !heroData.portrait().isBlank())
      embedBuilder.thumbnail(heroData.portrait());
